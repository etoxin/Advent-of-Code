...
const setupInstructions = instructions.split('\n').map(instruction => {
    return instruction.match(/move (\d+) from (\d+) to (\d+)/).slice(1).map(Number)
})

puzzleInput.reverse().forEach((row) => row.forEach((crate, column) => {
        if (crate === "") return;
...
setupInstructions.forEach(inst => {
    stacks[inst[2] - 1].push(...stacks[inst[1] - 1].splice(-Math.abs(inst[0]), inst[0]))
})
