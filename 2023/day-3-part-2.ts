...
lines.forEach((line, rowNumber) => {
  const partNumbers = [...line.matchAll(NUMBER_REGEX)].filter(([partNumber]) =>
    partNumber !== ""
  );
  partNumbers.forEach((pnRx) => {
    ...
    const surroundingCharacter = new Map([
      ...
      [
        "charactersAbove",
        lineAbove?.substring(numberPositionStart, numberPostionEnd + 1) ?? "",
      ],
      [
        "charactersBelow",
        lineBelow?.substring(numberPositionStart, numberPostionEnd + 1) ?? "",
      ],
    ]);
